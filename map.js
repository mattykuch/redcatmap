...
/*      console.log('d', d, 'event', event);*/
      ...
      d3.select("#tooltip .value")
        .text("Cat" + " " + valueFormat(d.properties.currentKey) ); 
}
...
var color = d3.scale.quantize()
        ...
        .range(['#008000','#ffa500','#ffff00','#ff0000']);


// Load in coverage score data
d3.csv("data/redcat_v2.csv", function(data) {
  ...
  d3.json('data/ug_districts2.geojson', function(error, features) {
    ...
    for (var i=0; i < data.length ; i++ ) {
      ...
      for (var j=0; j < features.features.length ; j++ ) {
        ...
        var featuresDistrict = features.features[j].properties.dist;

        if (dataDistrict == featuresDistrict) {

          //Copy the data value into the GeoJSON
          features.features[j].properties.currentKey = dataValue;
